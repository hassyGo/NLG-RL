...
class Corpus:
    ...
    def processBatchInfoNMT(self, batch, train, volatile = False):
        begin = batch[0]
        end = batch[1]
        ...
        batchInputSource = Variable(batchInputSource, volatile = volatile)
        batchInputSource = batchInputSource.cuda()
        ...
        batchInputTarget = Variable(batchInputTarget, requires_grad = False, volatile = volatile)
        batchTarget = Variable(batchTarget, requires_grad = False, volatile = volatile)

        return batchInputSource, lengthsSource, batchInputTarget, batchTarget, lengthsTarget, tokenCount, data, maxLen

    def processBatchInfoVocGen(self, batchData, train = True, smoothing = True, volatile = False):
        ...
        if batchData[0].targetText is not None and train:
            ...
            genVocTarget = Variable(genVocTarget, requires_grad = False, volatile = volatile)
            genVocTarget = genVocTarget.cuda()
        else:
            genVocTarget = None
        ...
        genVocInput = Variable(torch.LongTensor(genVocInput), requires_grad = False).cuda()
        genVocInputPos = Variable(torch.LongTensor(genVocInputPos), requires_grad = False).cuda()
