...
class EncDec(nn.Module):
    ...
    def forward(self, inputTarget, lengthsTarget, lengthsSource, hidden0Target, sourceH, device, output_list = None):
        ...
        finalHidden = torch.FloatTensor(batchSize, maxLen, self.targetEmbedDim).to(device)
        prevFinalHidden = torch.FloatTensor(batchSize, 1, self.targetEmbedDim).zero_().to(device)
        ...
        for i in range(maxLen):
            ...
            attentionScores = torch.FloatTensor(attentionScores_.size()).fill_(-1024.0).to(device)
    ...
    def sample(self, bosIndex, eosIndex, lengthsSource, targetEmbedding, sourceH, hidden0Target, device, useSmallSoftmax = False, fullSoftmax = False, output_list = None, train = False, greedyProb = 1.0, maxGenLen = 100):
        batchSize = sourceH.size(0)
        i = 1
        eosCount = 0
        targetWordIndices = torch.LongTensor(batchSize, maxGenLen).fill_(bosIndex).to(device)
        sampledIndices = torch.LongTensor(batchSize, maxGenLen).fill_(bosIndex).to(device)
        attentionIndices = torch.LongTensor(targetWordIndices.size()).to(device)
        targetWordLengths = torch.LongTensor(batchSize).zero_()
        fin = [False]*batchSize
        ...
        if train:
            finalHiddenAll = torch.FloatTensor(batchSize, maxGenLen, self.targetEmbedDim).to(device) # (B, Lt, Dt)
        ...
        if train:
            ...
            output = F.log_softmax(output, dim = 2) # the same size
